 ...
 *
 * 扩展：
 * 本题在复杂度上能够达到的程度完全取决于二叉树遍历的实现，如果一个二叉树遍历的实现在时间复杂度和空间复杂
 * 度上足够好，那么本题就可以做到在时间复杂度和空间复杂度上同样好。如果二叉树的节点数为 N，有没有时间复杂
 * 度为 O(N)、额外空间复杂度为 O(1) 的方法。既不用栈，也不用递归函数，只用有限的几个变量就可以实现，这样
 * 的遍历是有的。可以参考 "遍历二叉树的神级方法"，然后结合神级的遍历方法再重新实现这道题。
 *
 * @author Jiajing Li
